#!/usr/bin/env bash
#
# Generate EasyCommerce stubs for all available versions.
#
# This script:
# 1. Fetches all available EasyCommerce versions from WordPress.org
...
OUTPUT_FILE="$ROOT_DIR/easycommerce_versions.txt"
PLUGIN_NAME="easycommerce"
PLUGIN_API_URL="https://api.wordpress.org/plugins/info/1.0/easycommerce.json"
...
while IFS= read -r VERSION; do
    ...
    git commit -m "Generate stubs for EasyCommerce ${VERSION}" || true
    git tag "v${VERSION}"
