#!/usr/bin/env bash
#
# Generate EasyCommerce stubs for the latest versions.
#
# This script:
# 1. Fetches the latest EasyCommerce versions from WordPress.org
...
while IFS= read -r VERSION; do
    ...
    if git diff --cached --quiet; then
        ...
        git commit -m "Generate stubs for EasyCommerce ${VERSION}"
        git tag "v${VERSION}"
    fi
